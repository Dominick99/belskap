...
import { getAvatar, getAvatarMedia, mediaUrl } from "../avatar-data";

export default async function AvatarProfilePage({ params }: { params: Promise<{ avatarId: string }> }) {
  const { avatarId } = await params;
  const [avatar, media] = await Promise.all([getAvatar(avatarId), getAvatarMedia(avatarId)]);
  if (!avatar) notFound();
      ...
      <section className="profile-feed">
        ...
        {media.length ? (
            ...
            {media.map((item) => {
              ...
              return (
                <article className="media-tile" key={item.id}>
                  {src ? (
                    item.media_type === "video" ? <video aria-label={`${avatar.name} video`} muted preload="metadata" src={src} /> : <img alt={`${avatar.name} media`} src={src} />
                  ) : (
          ...
          <div className="media-empty">
            <h3>No media yet</h3>
            <p>Images and videos added to this avatar will build its profile grid.</p>
          </div>
        )}
      </section>
    </main>
